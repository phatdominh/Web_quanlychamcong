...
(function ($) {
    var Transfer = function (element, options) {
        ...
        // default zero item
        this.default_right_item_total_num_text = get_total_num_text(
            this.default_total_num_text_template,
            0,
            0
        );
        // item total number
        ...
        this.group_item_total_num = get_group_items_num(
            this.settings.groupDataArray,
            this.settings.groupArrayName
        );
        // use group
        ...
        this.transferDoubleGroupListUlClass =
            ".transfer-double-group-list-ul-" + this.id;
        // class selector for the transfer-double-group-list-li
        this.transferDoubleGroupListLiClass =
            ".transfer-double-group-list-li-" + this.id;
        ...
        this.transferDoubleGroupListLiUlLiClass =
            ".transfer-double-group-list-li-ul-li-" + this.id;
        ...
        this.transferDoubleSelectedListUlClass =
            ".transfer-double-selected-list-ul-" + this.id;
        // class selector for the transfer-double-selected-list-li
        this.transferDoubleSelectedListLiClass =
            ".transfer-double-selected-list-li-" + this.id;
                        ...
                        <div class="transfer-double-selected-list">
                            <div class="transfer-double-selected-list-header">
                                <div class="transfer-double-selected-list-search">
                                    ...
                                    <input class="transfer-double-selected-list-search-input"
                                     type="text" id="selectedListSearch_{{= self.id }}" placeholder="{{= self.searchPlaceholderText }}" value="" />
                                     </div>
                            </div>
                            <div class="transfer-double-selected-list-content">
                                <div class="transfer-double-selected-list-main">

                                    <ul class="transfer-double-selected-list-ul transfer-double-selected-list-ul-{{= self.id }}">
    ...
    Transfer.prototype.fill_data = function () {
        ...
        this.$element
            .find(this.transferDoubleListUlClass)
            .append(this.generate_left_items());

        this.$element.find(this.transferDoubleSelectedListUlClass).empty();
        this.$element
            .find(this.transferDoubleSelectedListUlClass)
            .append(this.generate_right_items());

        // render total num
        this.$element.find(this.totalNumLabelClass).empty();
        this.$element
            .find(this.totalNumLabelClass)
            .append(
                get_total_num_text(
                    this.default_total_num_text_template,
                    0,
                    this._data.get("left_total_count")
                )
            );
        ...
        this.$element
            .find(this.selectedTotalNumLabelClass)
            .append(
                get_total_num_text(
                    this.default_total_num_text_template,
                    0,
                    this._data.get("right_total_count")
                )
            );
    };
    ...
    Transfer.prototype.fill_group_data = function () {
        ...
        this.$element
            .find(this.transferDoubleGroupListUlClass)
            .append(this.generate_left_group_items());

        this.$element.find(this.transferDoubleSelectedListUlClass).empty();
        this.$element
            .find(this.transferDoubleSelectedListUlClass)
            .append(this.generate_right_group_items());

        var self = this;
        var left_total_count = 0;
        this._group_data.forEach(function (key, value) {
            left_total_count += value["left_total_count"];
            value["left_total_count"] == 0
                ? self.$element
                      .find("#" + key)
                      .prop("disabled", true)
                      .prop("checked", true)
        ...
        this.$element
            .find(this.groupTotalNumLabelClass)
            .append(
                get_total_num_text(
                    this.default_total_num_text_template,
                    0,
    ...
    Transfer.prototype.generate_left_items = function () {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            this._data.get("right_total_count") == undefined
            ...
            selected
                ...
                : void 0;
            !selected && disabled
                ? this._data.put("left_disabled_count", ++disabled_count)
                : void 0;
        ...
        this._data.put(
            "left_total_count",
            dataArray.length - this._data.get("right_total_count")
        );
    ...
    Transfer.prototype.generate_left_group_items = function () {
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            if (
                groupDataArray[i][groupArrayName] &&
                groupDataArray[i][groupArrayName].length > 0
            ) {
                var _value = {};
                _value["left_pre_selection_count"] = 0;
                _value["left_total_count"] =
                    groupDataArray[i][groupArrayName].length;
                this._group_data.put("group_" + i + "_" + this.id, _value);

                var groupTemplateCompiled = $.template(groupTemplate);
                html += groupTemplateCompiled({
                    self: this,
                ...
                for (
                    var j = 0;
                    j < groupDataArray[i][groupArrayName].length;
                    j++
                ) {
                    var selected =
                        groupDataArray[i][groupArrayName][j].selected || false;
                    var right_total_count =
                        this._data.get("right_total_count") || 0;
                    this._data.get("right_total_count") == undefined
                        ? this._data.put("right_total_count", right_total_count)
                        : void 0;
                    selected
                    ...
                    var groupItem = this._group_data.get(
                        "group_" + i + "_" + this.id
                    );
                    selected ? (groupItem["left_total_count"] -= 1) : void 0;

                    var groupItemTemplateCompiled =
                        $.template(groupItemTemplate);
                    html += groupItemTemplateCompiled({
                        self: this,
                        groupDataArray: groupDataArray,
                        i: i,
                        j: j,
    ...
    Transfer.prototype.generate_right_items = function () {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            if (dataArray[i].selected || false) {
                ...
                this._data.put("right_total_count", ++selected_count);
                html += this.generate_item(
                    this.id,
                    i,
                    dataArray[i][valueName],
                    dataArray[i][itemName],
                    disabled
                );
            }
        }
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            if (
                ...
                for (
                    ...
                    if (
                        groupDataArray[i][groupArrayName][j].selected ||
                        false
                    ) {
                        this._data.put("right_total_count", ++selected_count);
                        html += this.generate_group_item(
                            this.id,
                            i,
                            j,
    ...
    Transfer.prototype.left_checkbox_item_click_handler = function () {
        ...
        self.$element.on("click", self.checkboxItemClass, function () {
            ...
            var left_pre_selection_count = self._data.get(
                "left_pre_selection_count"
            );
            self._data.put(
                "left_pre_selection_count",
                left_pre_selection_count + pre_selection_num
            );
            self.$element
                .find(self.totalNumLabelClass)
                .text(
                    get_total_num_text(
                        self.default_total_num_text_template,
                        self._data.get("left_pre_selection_count"),
                        self._data.get("left_total_count")
                    )
                );
            ...
            if (
                self._data.get("left_pre_selection_count") ==
                self._data.get("left_total_count") -
                    self._data.get("left_disabled_count")
            ) {
                $(self.leftItemSelectAllId).prop("checked", true);
            }
        });
    ...
    Transfer.prototype.left_group_checkbox_item_click_handler = function () {
        ...
        self.$element.on("click", self.groupCheckboxItemClass, function () {
            ...
            var groupItem = self._group_data.get(
                "group_" + groupIndex + "_" + self.id
            );
            var left_pre_selection_count =
                groupItem["left_pre_selection_count"];
            groupItem["left_pre_selection_count"] =
                left_pre_selection_count + pre_selection_num;
            ...
            self.$element
                .find(self.groupTotalNumLabelClass)
                .text(
                    get_total_num_text(
                        self.default_total_num_text_template,
                        total_pre_selection_num,
                        remain_left_total_count
                    )
                );
            ...
            if (
                groupItem["left_pre_selection_count"] <
                groupItem["left_total_count"]
            ) {
                self.$element
                    .find("#group_" + groupIndex + "_" + self.id)
                    .prop("checked", false);
            } else if (
                groupItem["left_pre_selection_count"] ==
                groupItem["left_total_count"]
    ...
    Transfer.prototype.group_select_all_handler = function () {
        ...
        $(self.groupSelectAllClass).on("click", function () {
            ...
            var groups = self.$element.find(
                ".belongs-group-" + groupIndex + "-" + self.id
            );
            ...
            if ($(this).is(":checked")) {
                ...
                self._group_data.forEach(function (key, value) {
                    left_pre_selection_count +=
                        value["left_pre_selection_count"];
                    left_total_count += value["left_total_count"];
                });
                ...
                self._group_data.get($(this).prop("id"))[
                    "left_pre_selection_count"
                ] = 0;
    ...
    Transfer.prototype.group_item_select_all_handler = function () {
        ...
        $(self.groupItemSelectAllId).on("click", function () {
            var groupCheckboxItems = self.$element.find(
                self.groupCheckboxItemClass
            );
            var left_pre_selection_count = 0;
            var left_total_count = 0;
            if ($(this).is(":checked")) {
                for (var i = 0; i < groupCheckboxItems.length; i++) {
                    if (
                        groupCheckboxItems
                            ...
                            .eq(i)
                            .css("display") != "none" &&
                        !groupCheckboxItems.eq(i).is(":checked")
                    ) {
                        groupCheckboxItems.eq(i).prop("checked", true);
                        var groupIndex = groupCheckboxItems
                            .eq(i)
                            .prop("id")
                            .split("_")[1];
                        if (
                            !self.$element
                                .find(self.groupSelectAllClass)
                                .eq(groupIndex)
                                .is(":checked")
                        ) {
                            self.$element
    ...
    Transfer.prototype.left_group_items_search_handler = function () {
        ...
        $(self.groupItemSearcherId).on("keyup", function () {
            self.$element
                .find(self.transferDoubleGroupListUlClass)
                .css("display", "block");
            var transferDoubleGroupListLiUlLis = self.$element.find(
                self.transferDoubleGroupListLiUlLiClass
            );
            if ($(self.groupItemSearcherId).val() == "") {
                for (
                    var i = 0;
                    i < transferDoubleGroupListLiUlLis.length;
                    i++
                ) {
                    if (
                        !transferDoubleGroupListLiUlLis
                            .eq(i)
                            .hasClass("selected-hidden")
                    ) {
                        transferDoubleGroupListLiUlLis
                            .eq(i)
                            .parent("ul")
            ...
            // Mismatch
            self.$element
                .find(self.transferDoubleGroupListLiClass)
                .css("display", "none");
            ...
            for (var j = 0; j < transferDoubleGroupListLiUlLis.length; j++) {
                if (
                    ...
                    transferDoubleGroupListLiUlLis
                        ...
                        .text()
                        .trim()
                        .substr(0, $(self.groupItemSearcherId).val().length)
                        .toLowerCase() ==
                        $(self.groupItemSearcherId).val().toLowerCase()
                ) {
                    transferDoubleGroupListLiUlLis
                        .eq(j)
                        .parent("ul")
                    ...
                    if (!checkboxItems.eq(i).prop("disabled")) {
                        if (
                            checkboxItems
                                ...
                                .parent("li")
                                .css("display") != "none" &&
                            !checkboxItems.eq(i).is(":checked") &&
                            !checkboxItems.eq(i).prop("disabled")
                        ) {
    ...
    Transfer.prototype.right_item_select_all_handler = function () {
        ...
        $(self.rightItemSelectAllId).on("click", function () {
            var checkboxSelectedItems = self.$element.find(
                self.checkboxSelectedItemClass
            );
            if ($(this).is(":checked")) {
                self._data.put("right_pre_selection_count", 0);
                var right_pre_selection_count = self._data.get(
                    "right_pre_selection_count"
                );
                for (var i = 0; i < checkboxSelectedItems.length; i++) {
                    if (
                        checkboxSelectedItems
                            ...
                            .parent("li")
                            .css("display") != "none" &&
                        !checkboxSelectedItems.eq(i).prop("disabled")
                    ) {
                ...
                if (
                    self._data.get("right_pre_selection_count") <
                    self._data.get("right_total_count")
                ) {
                ...
                if (
                    self._data.get("right_pre_selection_count") ==
                    self._data.get("right_total_count") -
                        self._data.get("right_disabled_count")
                ) {
                    $(self.rightItemSelectAllId).prop("checked", true);
                }
            } else {
                for (var i = 0; i < checkboxSelectedItems.length; i++) {
                    if (
                        checkboxSelectedItems
                            ...
                            .parent("li")
                            .css("display") != "none" &&
                        checkboxSelectedItems.eq(i).is(":checked")
                    ) {
            ...
            self.$element
                .find(self.selectedTotalNumLabelClass)
                .text(
                    get_total_num_text(
                        self.default_total_num_text_template,
                        self._data.get("right_pre_selection_count"),
    ...
    Transfer.prototype.left_items_search_handler = function () {
        ...
        $(self.itemSearcherId).on("keyup", function () {
            var transferDoubleListLis = self.$element.find(
                self.transferDoubleListLiClass
            );
            self.$element
                .find(self.transferDoubleListUlClass)
                .css("display", "block");
            if ($(self.itemSearcherId).val() == "") {
                for (var i = 0; i < transferDoubleListLis.length; i++) {
                    if (
                        !transferDoubleListLis.eq(i).hasClass("selected-hidden")
                    ) {
                        self.$element
                            .find(self.transferDoubleListLiClass)
                            .eq(i)
                            .css("display", "block");
                    }
                }
                ...
                for (var j = 0; j < transferDoubleListLis.length; j++) {
                    if (
                        ...
                        transferDoubleListLis
                            ...
                            .toLowerCase()
                            .indexOf(
                                $(self.itemSearcherId).val().toLowerCase()
                            ) > -1
                    ) {
                ...
                for (var j = 0; j < transferDoubleListLis.length; j++) {
                    if (
                        ...
                        transferDoubleListLis
                            ...
                            .text()
                            .trim()
                            .substr(0, $(self.itemSearcherId).val().length)
                            .toLowerCase() ==
                            $(self.itemSearcherId).val().toLowerCase()
                    ) {
                        transferDoubleListLis.eq(j).css("display", "block");
    ...
    Transfer.prototype.right_checkbox_item_click_handler = function () {
        ...
        self.$element.on("click", self.checkboxSelectedItemClass, function () {
            ...
            self._data.put(
                "right_pre_selection_count",
                right_pre_selection_count + pre_selection_num
            );
            self.$element
                .find(self.selectedTotalNumLabelClass)
                .text(
    ...
    Transfer.prototype.move_pre_selection_items_handler = function () {
        ...
        $(self.addSelectedButtonId).on("click", function () {
            if ($(this).hasClass("btn-arrow-active")) {
                self.isGroup
                    ? self.move_pre_selection_group_items()
                    : self.move_pre_selection_items();
                // callable
                applyCallable(self);
            }
        });
    ...
    Transfer.prototype.move_pre_selection_group_items = function () {
        var pre_selection_num = 0;
        var html = "";
        var groupCheckboxItems = this.$element.find(
            this.groupCheckboxItemClass
        );
        for (var i = 0; i < groupCheckboxItems.length; i++) {
            if (
                !groupCheckboxItems
                ...
                var labelText = this.$element
                    .find(this.groupCheckboxNameLabelClass)
                    .eq(i)
                    .text();
        ...
        if (pre_selection_num > 0) {
            var groupSelectAllArray = this.$element.find(
                this.groupSelectAllClass
            );
            ...
            var groupTotalNumLabel = this.$element.find(
                this.groupTotalNumLabelClass
            );
            groupTotalNumLabel.text(
                get_total_num_text(
                    this.default_total_num_text_template,
                    0,
                    remain_left_total_count
            ...
            var transferDoubleSelectedListUl = this.$element.find(
                this.transferDoubleSelectedListUlClass
            );
    ...
    Transfer.prototype.move_pre_selection_items = function () {
        ...
        for (var i = 0; i < checkboxItems.length; i++) {
            if (
                checkboxItems.eq(i).parent("div").parent("li").css("display") !=
                    "none" &&
                checkboxItems.eq(i).is(":checked")
            ) {
                ...
                var labelText = self.$element
                    .find(self.checkboxItemLabelClass)
                    .eq(i)
                    .text();
        ...
        if (pre_selection_num > 0) {
            ...
            if (
                self._data.get("left_total_count") == 0 ||
                self._data.get("left_disabled_count") ==
                    self._data.get("left_total_count")
            ) {
                $(self.leftItemSelectAllId)
                    .prop("checked", true)
                    .prop("disabled", "disabled");
            }
            ...
            $(self.addSelectedButtonId).removeClass("btn-arrow-active");
            self.$element
                .find(self.transferDoubleSelectedListUlClass)
                .append(html);
        }
    };
    ...
    Transfer.prototype.move_selected_items_handler = function () {
        ...
        $(self.deleteSelectedButtonId).on("click", function () {
            if ($(this).hasClass("btn-arrow-active")) {
                self.isGroup
                    ? self.move_selected_group_items()
    ...
    Transfer.prototype.move_selected_group_items = function () {
        var pre_selection_num = 0;
        var checkboxSelectedItems = this.$element.find(
            this.checkboxSelectedItemClass
        );
        for (var i = 0; i < checkboxSelectedItems.length; ) {
            var another_checkboxSelectedItems = this.$element.find(
                this.checkboxSelectedItemClass
            );
            if (
                another_checkboxSelectedItems
                    .eq(i)
                    .parent("div")
                    .parent("li")
                    .css("display") != "none" &&
                another_checkboxSelectedItems.eq(i).is(":checked")
            ) {
                var checkboxSelectedItemId = another_checkboxSelectedItems
                    .eq(i)
                    .attr("id");
                ...
                another_checkboxSelectedItems
                    ...
                    .remove();
                this.$element
                    .find("#group_" + groupIndex + "_" + this.id)
                    .prop("checked", false)
                    .removeAttr("disabled");
                this.$element
                    .find(
                ...
                var left_total_count = groupItem["left_total_count"];
                var right_pre_selection_count = this._data.get(
                    "right_pre_selection_count"
                );
                var right_total_count = this._data.get("right_total_count");
                groupItem["left_total_count"] = ++left_total_count;
    ...
    Transfer.prototype.move_selected_items = function () {
        ...
        for (
            var i = 0;
            i < self.$element.find(self.checkboxSelectedItemClass).length;
            ...
            if (
                ...
                var index = checkboxSelectedItems
                    .eq(i)
                    .attr("id")
                    .split("_")[1];
                checkboxSelectedItems.parent("div").parent("li").eq(i).remove();
                self.$element
                    .find(self.checkboxItemClass)
                    .eq(index)
                    .prop("checked", false);
                self.$element
                    .find(self.transferDoubleListLiClass)
        ...
        if (
            self._data.get("right_total_count") == 0 ||
            self._data.get("right_pre_selection_count") ==
                self._data.get("right_total_count") -
                    self._data.get("right_disabled_count")
        ) {
    ...
    Transfer.prototype.right_items_search_handler = function () {
        ...
        $(self.selectedItemSearcherId).keyup(function () {
            var transferDoubleSelectedListLis = self.$element.find(
                self.transferDoubleSelectedListLiClass
            );
            self.$element
                .find(self.transferDoubleSelectedListUlClass)
                .css("display", "block");
            ...
            if (self.settings.wildcardSearch) {
                for (var i = 0; i < transferDoubleSelectedListLis.length; i++) {
                    if (
                        transferDoubleSelectedListLis
                            .eq(i)
                            .text()
                            .trim()
                            .toLowerCase()
                            .indexOf(
                                $(self.selectedItemSearcherId)
                                    .val()
                                    .toLowerCase()
                            ) > -1
                    ) {
                ...
                for (var i = 0; i < transferDoubleSelectedListLis.length; i++) {
                    if (
                        transferDoubleSelectedListLis
                            ...
                            .substr(
                                0,
                                $(self.selectedItemSearcherId).val().length
                            )
                            .toLowerCase() ==
                        $(self.selectedItemSearcherId).val().toLowerCase()
                    ) {
                        transferDoubleSelectedListLis
                            .eq(i)
                            .css("display", "block");
     ...
     * generate item
     */
    Transfer.prototype.generate_item = function (
        id,
        index,
        value,
        labelText,
        ...
        var template = parseHTMLTemplate(function () {
            ...
            <li class="transfer-double-selected-list-li  transfer-double-selected-list-li-{{= id }} .clearfix">
                <div class="checkbox-group">
                    ...
                    <input type="hidden" value="{{= value}}" name="employee[]">
                    ...
                    <label class="selectEmployee checkbox-selected-name-{{= id }}" for="selectedCheckbox_{{= index }}_{{= id }}">{{= labelText }}</label>
     ...
     * generate group item
     */
    Transfer.prototype.generate_group_item = function (
        id,
        groupIndex,
        itemIndex,
        value,
    ...
    function applyCallable(transfer) {
        if (
            Object.prototype.toString.call(transfer.settings.callable) ===
            "[object Function]"
        ) {
    ...
    function get_selected_items(transfer) {
        var selected = [];
        var transferDoubleSelectedListLiArray = transfer.$element.find(
            transfer.transferDoubleSelectedListLiClass
        );
        for (var i = 0; i < transferDoubleSelectedListLiArray.length; i++) {
            var checkboxGroup = transferDoubleSelectedListLiArray
                .eq(i)
                .find(".checkbox-group");

            var item = {};
            item[transfer.settings.itemName] = checkboxGroup
                .find("label")
                .text();
            item[transfer.settings.valueName] = checkboxGroup
                .find("input")
                .val();
            selected.push(item);
        }
    ...
    function get_group_items_num(groupDataArray, groupArrayName) {
        ...
        for (var i = 0; i < groupDataArray.length; i++) {
            ...
            if (groupItemData && groupItemData.length > 0) {
                group_item_total_num =
                    group_item_total_num + groupItemData.length;
            }
        }
